...
resource "azurerm_virtual_network" "network" {
  ...
  dynamic "ddos_protection_plan" {
    ...
    content {
      ...
      enable = var.ddos_protection.enabled
    }
  }
  tags = var.tags
}

resource "azurerm_virtual_network_dns_servers" "network_dns" {
  #checkov:skip=CKV_AZURE_182:When using firewall for DNS queries only one IP address is available
  for_each           = length(var.dns_servers) == 0 ? [] : toset(var.dns_servers)
